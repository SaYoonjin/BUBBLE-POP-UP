        ...
        {menus.map((menu) => {
          ...
          return (
            <button
              ...
              <span
                className={`text-4xl md:text-5xl mb-3 transition-all ${
                  isSelected ? "" : "filter grayscale opacity-80 group-hover:grayscale-0 group-hover:opacity-100"
                }`}
              >
                {menu.emoji}
              </span>
              <span className={`text-sm md:text-[15px] font-medium text-center leading-tight ${isSelected ? "font-bold text-slate-900" : "text-slate-600"}`}>
                {menu.name}
              </span>
            </button>
          );
